...
namespace Sound2Light.Views.Controls.Units
{
    public partial class UnitCapture : UserControl
    {
        ...
        private void LeftVuMeterCanvas_PaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            ...
            canvas.Clear();
            DrawLeds(canvas, _viewModel.LeftActiveLed, e.Info.Width, e.Info.Height);
        }
        ...
        private void RightVuMeterCanvas_PaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            ...
            var canvas = e.Surface.Canvas;
            canvas.Clear();
            DrawLeds(canvas, _viewModel.RightActiveLed, e.Info.Width, e.Info.Height);
        }
        ...
        {
            if (db <= -72f) return 0;
            if (db >= 6f) return LedCount - 1;

            if (db < -40f)
            {
                float fraction = (db + 72f) / 32f;
                return (int)(fraction * 12);
            }
            else
            {
                float fraction = (db + 40f) / 46f;
                return 12 + (int)(fraction * 36);
            }
        }

        private void DrawLeds(SKCanvas canvas, int activeIndex, int width, int height)
        {
